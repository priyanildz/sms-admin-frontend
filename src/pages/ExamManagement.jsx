...
const ExamManagement = () => {
  ...
  const fetchSchedules = async () => {
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}api/etimetable`, {
        headers: {
          auth: AUTH_HEADER,
  ...
  const handleDeleteTimetable = async (id, examtype, standard) => {
    if (
      !window.confirm(
        `Are you sure you want to DELETE the entire ${examtype} timetable for Standard ${standard}? This action cannot be undone.`,
      )
    ) {
      return;
    }
  ...
  const handleDeleteEntry = (indexToDelete) => {
    if (
      !window.confirm("Are you sure you want to delete this subject entry?")
    ) {
      return;
    }
    const newTimetable = editFormData.timetable.filter(
  ...
  const handleUpdateTimetable = async (e) => {
    ...
    if (editFormData.timetable.length === 0) {
      alert(
        "Timetable cannot be empty. Use 'Delete Timetable' if you wish to remove the schedule entirely.",
      );
      return;
    }

    // Basic validation check (can be expanded)
    const isValid = editFormData.timetable.every(
      (entry) =>
        entry.date && entry.subject && entry.starttime && entry.endtime,
    );
    if (!isValid) {
      alert(
        "Please ensure all date, subject, start time, and end time fields are filled for every entry.",
      );
      return;
    }
                        ...
                        >
                          ...
                          <td className="p-2">
                            <input
                              ...
                              value={entry.subject}
                              onChange={(e) =>
                                handleEditChange(e, index, "subject")
                              }
                              className="w-full border rounded px-1 py-1 text-xs"
                              required
                            />
                          ...
                          <td className="p-2">
                            <input
                              ...
                              onChange={(e) =>
                                handleEditChange(e, index, "starttime")
                              }
                              className="w-full border rounded px-1 py-1 text-xs"
                              required
                            />
                          ...
                          <td className="p-2">
                            <input
                              ...
                              value={entry.endtime}
                              onChange={(e) =>
                                handleEditChange(e, index, "endtime")
                              }
                              className="w-full border rounded px-1 py-1 text-xs"
                              required
                            />
