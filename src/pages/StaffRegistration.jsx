...
export default function StaffRegistration() {
  ...
  const uploadToCloudinary = async (file, docType, customName) => {
    ...
    return axios.post(
      "https://api.cloudinary.com/v1_1/dyloa2svi/auto/upload",
      uploadFormData,
      { timeout: 60000 } // Extended timeout for mobile networks
  ...
  const handleSubmit = async () => {
    if (!validateForm()) return alert("Please check required fields");
    if (documents.length < 2) return alert("Please upload at least 2 documents");
    if (!isDeclared) return alert("Please accept the terms");
    ...
    try {
      ...
      // Parallelize all uploads to prevent sequential timeout issues on mobile
      const photoPromise = photo ? uploadToCloudinary(photo, "photos", `${formData.firstname}_photo`) : Promise.resolve(null);
      ...
      const [photoUrl, uploadedDocs] = await Promise.all([photoPromise, Promise.all(docPromises)]);

      submissionData.photo = photoUrl;
      submissionData.documentsurl = uploadedDocs;
      ...
      // Improved error message for mobile users
      const errorMsg = err.message.includes("timeout") 
        ? "Upload timed out. Please try on a faster connection." 
        : (err.response?.data?.error || "Network error. If you are on a phone, try re-selecting your files.");
      alert(`Error: ${errorMsg}`);
    } finally {
      setIsSubmitting(false);
    }
  ...
  const getLabel = (type) => {
    const labels = { photo: "Staff Photo", aadhaar: "Aadhaar Card", resume: "Resume/CV", certificates: "Certificates", experience: "Experience Letters" };
    return labels[type] || "";
  };

  if (formSubmitted) return (
    <MainLayout>
        <div className="p-8 text-center bg-white shadow rounded-2xl max-w-xl mx-auto mt-10">
            <h2 className="text-2xl font-bold text-green-600">Registered Successfully!</h2>
            <p className="mt-2">Staff ID: {formData.staffid}</p>
        </div>
    </MainLayout>
  );
    ...
    <MainLayout>
      <div className="p-8">
        <div className="bg-white rounded-2xl shadow p-6">
          ...
          {/* Main Form Body */}
          <div className="grid grid-cols-1 md:grid-cols-[3fr_1fr] gap-6 mb-6">
            <div className="rounded-lg p-6 shadow-lg bg-white w-full border">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                ...
                <SelectField label="Marital Status *" options={["", "Single", "Married", "Divorced", "Widowed"]} value={formData.maritalstatus} onChange={(val) => handleChange("maritalstatus", val)} />
                <SelectField label="Blood Group *" options={["", "A+", "A-", "B+", "B-", "O+", "O-", "AB+", "AB-"]} value={formData.bloodgroup} onChange={(val) => handleChange("bloodgroup", val)} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <SelectField label="Gender *" options={["", "Male", "Female", "Other"]} value={formData.gender} onChange={(val) => handleChange("gender", val)} />
                <SelectField label="Nationality *" options={["", "Indian", "Other"]} value={formData.nationality} onChange={(val) => handleChange("nationality", val)} />
                <SelectField label="Category *" options={["", "General", "OBC", "SC", "ST", "Other"]} value={formData.category} onChange={(val) => handleChange("category", val)} />
              </div>
              <div className="mt-6"><InputField label="Aadhar Number *" value={formData.aadharno} onChange={(val) => handleChange("aadharno", val)} /></div>
            </div>

            {/* Photo Section */}
            <div className="w-full flex flex-col items-center p-6 rounded-lg shadow-lg bg-white self-start border">
              <div className="w-full h-48 bg-gray-200 rounded mb-2 flex items-center justify-center relative overflow-hidden shadow-inner">
                {photoPreview ? <img src={photoPreview} alt="Preview" className="w-full h-full object-cover" /> : <span className="text-sm text-gray-500">Photo Preview</span>}
              </div>
              <label className="bg-blue-400 text-white text-sm px-4 py-2 rounded cursor-pointer mb-2">Choose Photo<input type="file" accept="image/*" onChange={handlePhotoChange} className="hidden" /></label>
              <div className="w-full border-t pt-4 mt-2">
                <label className="block text-sm font-medium text-gray-700 mb-2 text-center">Staff Status</label>
                <div className="flex items-center justify-center">
                  <button type="button" onClick={handleStatusToggle} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.status ? "bg-green-500" : "bg-gray-300"}`}>
          ...
          {/* Address and Bank Section */}
          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Address & Bank Details</h4></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <InputField label="Phone *" value={formData.phoneno} onChange={(val) => handleChange("phoneno", val)} />
              ...
              <InputField label="IFSC *" value={formData.ifccode} onChange={(val) => handleChange("ifccode", val)} />
            </div>
          </div>

          {/* Role and Documents Section */}
          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Role & Upload Documents</h4></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
            ...
            <div className="space-y-4 mt-6">
              <select className="block w-full border border-gray-300 rounded px-3 py-2" value={selectedDoc} onChange={(e) => setSelectedDoc(e.target.value)}>
                <option value="">Select Document Type</option>
                <option value="resume">Resume/CV</option>
                <option value="aadhaar">Aadhaar Card</option>
              </select>
              {selectedDoc && <input type="file" onChange={handleDocumentUpload} data-doc-type={selectedDoc} className="block w-full border border-gray-300 rounded px-3 py-2" />}
              <div className="space-y-2">{documents.map((doc, index) => (<div key={index} className="bg-gray-50 p-2 rounded text-sm">📄 {doc.name} ({doc.type})</div>))}</div>
            </div>
          </div>

          <div className="flex items-center gap-2 mb-8"><input type="checkbox" checked={isDeclared} onChange={(e) => setIsDeclared(e.target.checked)} /><label>I agree to the terms and conditions</label></div>

          <div className="flex justify-center">
            <button onClick={handleSubmit} disabled={!isDeclared || isSubmitting} className={`px-8 py-3 text-white font-semibold rounded-lg ${isDeclared && !isSubmitting ? "bg-blue-500" : "bg-gray-400"}`}>
