...
export default function StaffRegistration() {
  ...
  const fetchBankDetailsByIFSC = async (ifsc) => {
    setErrors((prev) => ({ ...prev, ifccode: "Fetching details..." }));
    try {
      const response = await axios.get(`${IFSC_API_URL}${ifsc}`);
      if (response.data && response.data.BANK) {
      ...
      setErrors(prev => ({ ...prev, ifccode: "Invalid IFSC Code." }));
    }
  };
  ...
  const fetchAddressByPin = async (pincode) => {
    ...
    try {
      ...
      if (response.data && response.data[0].Status === 'Success') {
        const postOffice = response.data[0].PostOffice[0];
        setFormData(prev => ({ ...prev, city: postOffice.Region, district: postOffice.District, state: postOffice.State, country: 'India' }));
        setIsAddressAutoPopulated(true);
  ...
  const validateForm = () => {
    const requiredFields = ["firstname", "lastname", "dob", "gender", "nationality", "phoneno", "emailaddress", "addressline1", "postalcode", "aadharno", "highestqualification", "yearofpassing", "universityname", "bankname", "accno", "ifccode", "panno", "position", "dept", "joiningdate"];
    let isValid = true;
    requiredFields.forEach((field) => {
  ...
  const handleDocumentUpload = (e) => {
    ...
    const newDocuments = files.map((file) => ({ name: file.name, type: selectedDocType, file: file }));
    setDocuments((prev) => [...prev.filter((doc) => doc.type !== selectedDocType), ...newDocuments]);
    e.target.value = "";
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (photo) {
        ...
        submissionData.photo = await uploadToCloudinary(photo, "photos", `${formData.firstname}_photo`);
      }

      // 2. Document Uploads (Now Optional)
      const documentUrls = [];
      if (documents.length > 0) {
        console.log(`Uploading ${documents.length} documents...`);
        for (const doc of documents) {
          const url = await uploadToCloudinary(doc.file, doc.type, `${formData.firstname}_${doc.type}`);
      ...
      // 3. Final API Call
      console.log("Sending data to Backend API:", `${API_BASE_URL}api/addstaff`);
      ...
      console.error("Submission Failed Error Object:", err);
      if (err.response) {
        ...
        alert("Network Error: Could not connect to the server. Check your internet or API URL.");
      } else {
  ...
  const getLabel = (type) => {
    const labels = { photo: "Photo", aadhaar: "Aadhaar", resume: "Resume", certificates: "Certificates", experience: "Experience" };
    return labels[type] || "";
  };
    ...
    return (
      <MainLayout>
        <div className="p-8 text-center bg-white rounded-2xl shadow max-w-xl mx-auto mt-10">
          ...
          <p className="text-gray-600">Registration has been completed.</p>
        </div>
      </MainLayout>
    );
  }
    ...
    <MainLayout>
      <div className="p-8">
        <div className="bg-white rounded-2xl shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[3fr_1fr] gap-6 mb-6">
            <div className="rounded-lg p-6 shadow bg-white">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                ...
                <InputField label="Aadhaar *" value={formData.aadharno} onChange={(val) => handleChange("aadharno", val)} />
              </div>
            </div>
            {/* Photo Section */}
            <div className="flex flex-col items-center p-6 rounded-lg shadow bg-white">
              <div className="w-full h-40 bg-gray-200 rounded mb-4 overflow-hidden">
                {photoPreview ? <img src={photoPreview} className="w-full h-full object-cover" /> : <div className="flex h-full items-center justify-center text-gray-400">Preview</div>}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6 p-6 shadow rounded-lg bg-white">
            <InputField label="Highest Qualification *" value={formData.highestqualification} onChange={(val) => handleChange("highestqualification", val)} />
            <InputField label="University *" value={formData.universityname} onChange={(val) => handleChange("universityname", val)} />
          </div>

          {/* Bank Section */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6 p-6 shadow rounded-lg bg-white">
          ...
          <div className="rounded-lg p-6 shadow bg-white mb-6">
            <h4 className="font-semibold mb-4">Upload Documents (Optional)</h4>
            <select className="border p-2 w-full mb-4 rounded" value={selectedDoc} onChange={(e) => setSelectedDoc(e.target.value)}>
              <option value="">Select Document Type</option>
              <option value="resume">Resume</option>
              <option value="aadhaar">Aadhaar Card</option>
              <option value="certificates">Certificates</option>
            </select>
            {selectedDoc && <input type="file" onChange={handleDocumentUpload} data-doc-type={selectedDoc} className="w-full" />}
            <div className="mt-4 space-y-1">
              {documents.map((d, i) => <div key={i} className="text-sm text-gray-600">📄 {d.name} ({d.type})</div>)}
            </div>
          </div>
          ...
          <div className="flex justify-center">
            <button onClick={handleSubmit} disabled={isSubmitting} className={`px-10 py-3 rounded-lg text-white ${isSubmitting ? 'bg-gray-400' : 'bg-blue-600'}`}>
