...
export default function StaffRegistration() {
  ...
  const [formData, setFormData] = useState({
    staffid: generateStaffId(),
    firstname: "", middlename: "", lastname: "", dob: "", maritalstatus: "",
    bloodgroup: "", gender: "", nationality: "", category: "", aadharno: "",
    phoneno: "", alternatephoneno: "", emailaddress: "", password: "teacher@123",
    addressline1: "", addressline2: "", city: "", postalcode: "", district: "",
    state: "", country: "", photo: "", highestqualification: "", yearofpassing: "",
    specialization: "", certificates: "", universityname: "", totalexperience: "",
    designation: "", previousemployer: "", subjectstaught: "", reasonforleaving: "",
    position: "", dept: "", preferredgrades: "", joiningdate: "", bankname: "",
    branchname: "", accno: "", ifccode: "", panno: "", transportstatus: "",
    pickuppoint: "", droppoint: "", modetransport: "", status: true, documentsurl: [],
  });
  ...
  const fetchBankDetailsByIFSC = async (ifsc) => {
    try {
      const response = await axios.get(`${IFSC_API_URL}${ifsc}`);
      if (response.data && response.data.BANK) {
        setFormData(prev => ({ ...prev, bankname: response.data.BANK, branchname: response.data.BRANCH }));
        setIsBankAutoPopulated(true);
      }
    } catch (error) { setIsBankAutoPopulated(false); }
  };
  ...
  const fetchAddressByPin = async (pincode) => {
    try {
      ...
      if (response.data && response.data[0].Status === 'Success') {
        ...
        setFormData(prev => ({ ...prev, city: po.Region || po.Name, district: po.District, state: po.State, country: 'India' }));
        setIsAddressAutoPopulated(true);
      }
    } catch (error) { setIsAddressAutoPopulated(false); }
  };

  const validateField = (field, value) => {
    let errorMsg = "";
    if (!value && ["firstname", "lastname", "dob", "gender", "aadharno", "phoneno", "emailaddress", "addressline1", "postalcode", "highestqualification", "yearofpassing", "universityname", "bankname", "branchname", "accno", "ifccode", "panno", "position", "dept", "joiningdate"].includes(field)) {
  ...
  const handleChange = (field, value) => {
    let newFormData = { ...formData, [field]: value };
    if (field === 'postalcode' && value.length === 6) fetchAddressByPin(value);
    if (field === 'ifccode' && value.length === 11) fetchBankDetailsByIFSC(value);
    setFormData(newFormData);
    validateField(field, value);
  };
  ...
  const handleDocumentUpload = (e) => {
    ...
    const newDocs = files.map(file => ({ name: file.name, type: selectedDocType, file }));
    setDocuments(prev => [...prev.filter(d => d.type !== selectedDocType), ...newDocs]);
    e.target.value = "";
  };

  const handleStatusToggle = () => handleChange("status", !formData.status);

  const handleSubmit = async () => {
    if (!validateForm()) return alert("Check required fields");
    if (documents.length < 2) return alert("Upload at least 2 documents");
    if (!isDeclared) return alert("Accept terms");
    ...
    try {
      const submissionData = { ...formData };
      Object.keys(submissionData).forEach(key => {
        if (submissionData[key] === "" || submissionData[key] === null) delete submissionData[key];
      });
      ...
      if (photo) {
        uploadPromises.push(
          uploadToCloudinary(photo, "photos", `${formData.firstname}_photo`)
            .then(url => { submissionData.photo = url; })
        );
      }
      ...
      if (documents.length > 0) {
        const docPromises = documents.map(doc => 
          uploadToCloudinary(doc.file, doc.type, `${formData.firstname}_${doc.type}`)
            .then(url => ({ url, type: doc.type, name: doc.name }))
        );
        uploadPromises.push(
          Promise.all(docPromises).then(urls => { submissionData.documentsurl = urls; })
        );
      ...
      const response = await axios.post(`${API_BASE_URL}api/addstaff`, submissionData, {
        headers: { "Content-Type": "application/json", auth: AUTH_HEADER },
        timeout: 45000 // Give server 45s to handle DB writes
      });
      ...
      if (err.response) {
        ...
        // RECOVERY LOGIC: If duplicate Aadhaar/ID, we generate a fresh ID for the next attempt
        // Note: If the Aadhaar is already in DB, you MUST change the Aadhaar number in the form to test again.
        if (err.response.status === 409) {
           setFormData(prev => ({ ...prev, staffid: generateStaffId() }));
        }
      } else {
        alert("Upload Timeout or Network Issue. The data might have been saved. Refresh the staff list to check.");
      }
    } finally {
      setIsSubmitting(false);
    }
  ...
  const getLabel = (type) => {
    const labels = { photo: "Staff Photo", aadhaar: "Aadhaar Card", resume: "Resume/CV", certificates: "Certificates", experience: "Letters" };
    return labels[type] || "";
  };

  if (formSubmitted) return <MainLayout><div className="p-8 text-center bg-white shadow rounded-2xl max-w-xl mx-auto mt-10"><h2>Registered!</h2><p>Staff ID: {formData.staffid}</p></div></MainLayout>;

  return (
    <MainLayout>
      <div className="p-8">
        <div className="bg-white rounded-2xl shadow p-6">
          <h4 className="text-2xl font-semibold mb-6 text-center">Staff Registration</h4>
          
          {/* Details Section */}
          <div className="grid grid-cols-1 md:grid-cols-[3fr_1fr] gap-6 mb-6">
            <div className="rounded-lg p-6 shadow-lg bg-white w-full border border-gray-100">
              <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Staff Details</h4></div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                ...
                <SelectField label="Marital Status *" options={["", "Single", "Married", "Divorced"]} value={formData.maritalstatus} onChange={(val) => handleChange("maritalstatus", val)} />
                <SelectField label="Blood Group *" options={["", "A+", "A-", "B+", "B-", "O+", "O-"]} value={formData.bloodgroup} onChange={(val) => handleChange("bloodgroup", val)} />
              </div>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
                <SelectField label="Gender *" options={["", "Male", "Female"]} value={formData.gender} onChange={(val) => handleChange("gender", val)} />
                <SelectField label="Nationality *" options={["", "Indian", "Other"]} value={formData.nationality} onChange={(val) => handleChange("nationality", val)} />
                <SelectField label="Category *" options={["", "General", "OBC", "SC", "ST"]} value={formData.category} onChange={(val) => handleChange("category", val)} />
              </div>
              <div className="mt-6"><InputField label="Aadhar Number *" value={formData.aadharno} onChange={(val) => handleChange("aadharno", val)} /></div>
            </div>

            <div className="w-full flex flex-col items-center p-6 rounded-lg shadow-lg bg-white self-start border border-gray-100">
              <div className="w-full h-48 bg-gray-100 rounded mb-2 flex items-center justify-center relative overflow-hidden shadow-inner">
                {photoPreview ? <img src={photoPreview} alt="Preview" className="w-full h-full object-cover" /> : <span className="text-xs text-gray-400 text-center">Photo Preview</span>}
              </div>
              <label className="bg-blue-400 text-white text-xs px-4 py-2 rounded cursor-pointer hover:bg-blue-600 transition">Choose Photo<input type="file" accept="image/*" onChange={handlePhotoChange} className="hidden" /></label>
              <div className="w-full border-t pt-4 mt-4 text-center">
                <label className="text-xs font-medium text-gray-500 mb-2 block uppercase tracking-wider">Status</label>
                <button type="button" onClick={handleStatusToggle} className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${formData.status ? "bg-green-500" : "bg-gray-300"}`}><span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${formData.status ? "translate-x-6" : "translate-x-1"}`} /></button>
              </div>
            </div>
          </div>

          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border border-gray-100">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Contact & Address</h4></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <InputField label="Phone Number *" value={formData.phoneno} onChange={(val) => handleChange("phoneno", val)} />
              <InputField label="Email Address *" type="email" value={formData.emailaddress} onChange={(val) => handleChange("emailaddress", val)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
          ...
          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border border-gray-100">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Employment Details</h4></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <InputField label="Qualification *" value={formData.highestqualification} onChange={(val) => handleChange("highestqualification", val)} />
              <InputField label="University *" value={formData.universityname} onChange={(val) => handleChange("universityname", val)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <SelectField label="Position *" options={POSITIONS} value={formData.position} onChange={(val) => handleChange("position", val)} />
              <InputField label="Joining Date *" type="date" value={formData.joiningdate} onChange={(val) => handleChange("joiningdate", val)} />
            </div>
          </div>

          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border border-gray-100">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Bank Details</h4></div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <InputField label="Bank Name *" value={formData.bankname} onChange={(val) => handleChange("bankname", val)} readOnly={isBankAutoPopulated} className={isBankAutoPopulated ? 'bg-gray-50' : ''} />
              <InputField label="Branch Name *" value={formData.branchname} onChange={(val) => handleChange("branchname", val)} readOnly={isBankAutoPopulated} className={isBankAutoPopulated ? 'bg-gray-50' : ''} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
              <InputField label="Account No *" value={formData.accno} onChange={(val) => handleChange("accno", val)} />
              <InputField label="IFSC Code *" value={formData.ifccode} onChange={(val) => handleChange("ifccode", val)} />
            </div>
            <div className="mt-6"><InputField label="PAN Number *" value={formData.panno} onChange={(val) => handleChange("panno", val)} /></div>
          </div>

          <div className="rounded-lg p-6 shadow-lg bg-white mb-6 border border-gray-100">
            <div className="-mt-6 -mx-6 bg-blue-400 text-white px-4 py-2 rounded-t"><h4 className="text-xl font-semibold">Upload Documents (Min 2)</h4></div>
            <div className="space-y-6 mt-6">
              <select className="block w-full border border-gray-300 rounded px-3 py-2 text-sm" value={selectedDoc} onChange={(e) => setSelectedDoc(e.target.value)}>
                <option value="">Select Document Type</option>
                <option value="resume">Resume/CV</option>
                <option value="aadhaar">Aadhaar Card</option>
              </select>
              {selectedDoc && <input type="file" onChange={handleDocumentUpload} data-doc-type={selectedDoc} className="block w-full text-xs" />}
              <div className="flex flex-wrap gap-2">{documents.map((doc, index) => (<div key={index} className="bg-blue-50 border border-blue-200 px-3 py-1 rounded text-xs text-blue-700 flex items-center gap-1">📄 {doc.name} ({doc.type})</div>))}</div>
            </div>
          </div>

          <div className="flex items-center gap-2 mb-8"><input type="checkbox" checked={isDeclared} onChange={(e) => setIsDeclared(e.target.checked)} className="rounded" /><label className="text-sm text-gray-600">I declare all provided info is accurate.</label></div>

          <div className="flex justify-center">
            <button onClick={handleSubmit} disabled={!isDeclared || isSubmitting} className={`px-12 py-4 text-white font-bold rounded-xl shadow-lg transition-all ${isDeclared && !isSubmitting ? "bg-blue-500 hover:bg-blue-700" : "bg-gray-300 cursor-not-allowed"}`}>
              {isSubmitting ? "UPLOADING & SAVING..." : "REGISTER STAFF"}
            </button>
          </div>
        </div>
      </div>
