...
const EditTransportStaff = () => {
    ...
    const fetchStaffDetails = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${API_BASE_URL}api/staff-details/${currentStaffId}`,
          {
            headers: {
        ...
        const normalizedStaff = {
          ...
          designation:
            fetchedStaff.designation ||
            (fetchedStaff.driverName ? "Driver" : "Supervisor"),
          ...
          firstName:
            fetchedStaff.firstName ||
            fetchedStaff.fullName?.split(" ")[0] ||
            fetchedStaff.driverName?.split(" ")[0] ||
            "",
          lastName:
            fetchedStaff.lastName ||
            fetchedStaff.fullName?.split(" ").slice(-1)[0] ||
            fetchedStaff.driverName?.split(" ").slice(-1)[0] ||
            "",
        ...
        setIsLoading(false);
        alert(
          "Failed to load staff details for editing. Check API '/staff-details/:id'.",
        );
      }
    };
  ...
  const headerText = isEditing
    ? `Edit ${editFormData.designation} Details`
    : `View ${editFormData.designation} Details`;
  ...
  const validateEditForm = () => {
    /* Simplified validation logic here */ return true;
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}api/update-vsupervisior/${staff._id}`,
        editFormData,
        {
          headers: {
            auth: "ZjVGZPUtYW1hX2FuZHJvaWRfMjAyMzY0MjU=",
