...
const AcademicProxy = () => {
  ...
  const fetchAvailableStandardsAndDivisions = async () => {
    try {
      ...
      const sortedStandards = Array.from(standardsSet).sort(
      ...
      console.warn(
        "Could not fetch available standards/divisions from allotments.",
      );
      setAllotmentList([]);
  ...
  const filteredProxies = proxyList.filter((proxy) => {
    ...
    const fromTeacherName =
      `${proxy.fromteacher?.firstname || ""} ${proxy.fromteacher?.lastname || ""}`.toLowerCase();
    const toTeacherName =
      `${proxy.toteacher?.firstname || ""} ${proxy.toteacher?.lastname || ""}`.toLowerCase();
          ...
          <div className="flex gap-6 pt-4 pb-2">
            ...
            <div className="w-48">
              ...
              >
                ...
                {divisionStandardMap[selectedStd]?.map(
                  (
                    div, // Filter divisions by selected std
                  ) => (
                  ...
                  filteredProxies.map((proxy, idx) => (
                    <tr key={idx} className="hover:bg-gray-50">
                      <td className="px-4 py-2 border text-center">
                        {new Date(proxy.date).toLocaleDateString("en-GB")}
                      </td>
