...
const EditStudent = () => {
  ...
  const toggleActive = async () => {
    ...
    try {
      await axios.put(`${API_BASE_URL}api/edit-student/${id}`, { status: newStatus }, { headers: { auth: AUTH_HEADER } });
    ...
    onSectionChange={(id) => setActiveSection(id)}>
      <div className="h-full w-full p-6">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8">
            <div>
                <h3 className="text-2xl font-semibold">{isViewMode ? "Student Profile" : "Edit Student Profile"}</h3>
            </div>
            <div className="flex items-center gap-3">
                <span className={`text-lg font-semibold ${isActive ? "text-green-600" : "text-red-600"}`}>
                    {isActive ? "Active" : "Inactive"}
                </span>
                <button onClick={toggleActive} disabled={isLoadingStudent || isViewMode} className="relative">
                    <div className={"rounded-full p-[3px] " + (isActive ? "border-2 border-blue-500" : "border-2 border-gray-300")}
                        ...
                        <div className={`w-20 h-10 rounded-full transition-colors duration-200 flex items-center relative ${isActive ? "bg-green-500" : "bg-gray-300"}`}>
