...
export default function ExamManagement() {
  ...
  const studentsInSelectedClass = promoteFrom
    ? filteredStudents.filter(
        (student) => student.admission?.admissionstd === promoteFrom,
      )
    : filteredStudents;
  ...
  const handleSelectAll = () => {
    if (selectAll) {
      ...
      setSelectedStudents(
        new Set(studentsInSelectedClass.map((s) => s.studentid)),
      );
      setSelectAll(true);
    }
  };
  ...
  const handlePromotion = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}api/promote-students`,
        payload,
        {
          headers: {
            auth: "ZjVGZPUtYW1hX2FuZHJvaWRfMjAyMzY0MjU=",
      ...
      alert(
        `Promotion failed: ${error.response?.data?.message || error.message}`,
      );
    } finally {
      setLoading(false);
    }
        ...
        {promoteFrom && (
          <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
            <p className="text-yellow-800">
              Showing students from <strong>Class {promoteFrom}</strong> (
        ...
        {selectedStudents.size > 0 && (
          <div className="mt-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
            <p className="text-blue-700 font-medium">
              ...
              selected for promotion from <strong>Class {promoteFrom}</strong>{" "}
              to{" "}
              <strong>
                {promoteTo === "Graduated" ? "Graduated" : `Class ${promoteTo}`}
              </strong>
              .
            </p>
          </div>
