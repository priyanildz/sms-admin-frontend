...
const AddEvents = () => {
    ...
    // Create staff options list from the passed data
    ...
    useEffect(() => {
        ...
        // This prevents students from being in the selected list if their class is deselected.
        const availableIds = new Set(allStudents.map(s => s._id));
        setSelectedParticipants(prev => prev.filter(p => availableIds.has(p.value)));
    ...
    const validateForm = () => {
        const isCoreFormValid = (
            ...
            eventDetails.date &&
            selectedManager.length > 0 // Manager is now multi-select
        );
    ...
    const handleSubmit = async (e) => {
        ...
        if (!validateForm()) {
            setError("Please fill all required fields (Event Name, Date, Managed By) and ensure the date is current or future.");
            return;
        }
        setError("");
        setLoading(true);
                            ...
                            <div className="mb-4">
                                ...
                                <div className="flex items-center space-x-0">
                                    <Select
                                        ...
                                        value={selectedParticipants}
                                        onChange={setSelectedParticipants}
                                        // Use CustomMultiValue to hide participant names in input
                                        components={{ MultiValue: CustomMultiValue }} 
                                        className="basic-multi-select flex-1 w-full"
                                        classNamePrefix="select"
                                        placeholder={
                                        ...
                                        isLoading={loadingStudents}
                                        maxMenuHeight={200}
                                        styles={{ container: (base) => ({ ...base, flex: 1, minHeight: '38px' }) }}
                                    />
                                </div>
                            </div>
