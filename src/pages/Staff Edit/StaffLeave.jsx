      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg">
        ...
        <div className="p-6">
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 text-xs space-y-1 text-gray-600">
            ...
            <p>
              <strong>Leave Period:</strong> {fromDateDisplay} to
              {toDateDisplay}
            </p>
          </div>
        </div>
...
export default function StaffLeave({ staff }) {
  ...
  const [selectedRequest, setSelectedRequest] = useState(null); // --- Dynamic Fetching Logic ---

  const fetchLeaveRequests = useCallback(async () => {
    if (!staffId) {
      console.warn(
        "DEBUG: StaffLeave - Staff ID is missing, cannot fetch requests.",
      );
      setLeaveRequests([]);
      return;
    }
    ...
    try {
      console.log(
        `DEBUG: StaffLeave - Fetching ALL leave requests from /getrequests`,
      );
      const storedUsername = localStorage.getItem("username") || "System_User";
      const storedRole = localStorage.getItem("role") || "admin"; // FIX: Using imported API_BASE_URL
      ...
      const allRequests = response.data || []; // FILTER STEP: Filter the list to include only requests matching the current staffId
      const filteredRequests = allRequests.filter(
        (request) => String(request.staffid) === String(staffId),
      );
      console.log(
        `DEBUG: StaffLeave - Filtered down to ${filteredRequests.length} requests for ${staffId}`,
      );
      setLeaveRequests(filteredRequests);
    } catch (err) {
      const errorMsg = err.response?.data?.message || err.message;
      console.error(
        "❌ ERROR: StaffLeave - Failed to fetch leave requests:",
        errorMsg,
      );
      setError(errorMsg);
      setLeaveRequests([]);
  ...
  const handleAcceptReject = async (requestId, newStatus) => {
    ...
    alert(
      `Request ${requestToUpdate.subject} is marked as ${newStatus.toUpperCase()}. (Backend endpoint placeholder executed)`,
    );
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {leaveRequests.map((request, index) => {
            ...
            return (
              <div // Use MongoDB _id for unique key if available, otherwise use index
              ...
              >
                ...
                <div className="p-4 text-right flex flex-col justify-center items-end flex-shrink-0 space-y-1">
                  <div
                    className={`${getStatusColor(request.status)} font-semibold mb-1`}
                  >
