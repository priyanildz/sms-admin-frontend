...
const generateMonthOptions = (count = 6) => {
  const options = [];
  let currentDate = new Date(); // 1. Determine the starting month: current date minus (count - 1) months.
  let startDate = new Date(
    currentDate.getFullYear(),
    currentDate.getMonth() - (count - 1),
    1,
  );
  ...
  for (let i = 0; i < count; i++) {
    ...
    options.push(`${month} ${year}`); // 2. Move forward by one month for the next iteration
...
export default function StaffAttendance({ staff }) {
  const staffName =
    ...
    "Staff Member";
  const staffId = staff?.staffid; // Generate options and set default month
  const monthOptions = generateMonthOptions(6); // Set the default to the MOST RECENT month, which is the LAST element in the chronological array
  ...
  const [isLoading, setIsLoading] = useState(false); // Legend updated to your requested simplified statuses
  ...
  const getDayClass = (dateNum, status) => {
    ...
    switch (status) {
      ...
      default: // Default style for unrecorded days
  ...
  const fetchAttendance = useCallback(async () => {
    if (!staffId) {
      console.warn(
        "DEBUG: Attendance - Staff ID is missing, cannot fetch attendance.",
      );
    ...
    try {
      console.log(
        `DEBUG: Attendance - Fetching records for staff ${staffId}, Month: ${month}, Year: ${year}`,
      );
      const storedUsername = localStorage.getItem("username") || "System_User";
      const storedRole = localStorage.getItem("role") || "admin"; // Using the imported API_BASE_URL
      const response = await axios.get(
        `${API_BASE_URL}api/staff/${staffId}/attendance`,
        {
      ...
      console.log(
        "DEBUG: Attendance - Processed attendance records:",
        attendanceMap,
      );
      setAttendanceRecords(attendanceMap);
    } catch (error) {
      console.error(
        "❌ ERROR: Attendance - Failed to fetch attendance data.",
        error,
      );
      setAttendanceRecords({});
    } finally {
  ...
  useEffect(() => {
    fetchAttendance();
  }, [fetchAttendance]); // --- Calendar Generation Logic (UNCHANGED) ---

  const renderCalendar = () => {
    const { month: monthStr, year: yearStr } =
      getMonthYearFromState(selectedMonthYear);
    ...
    const calendarCells = []; // 1. Fill initial padding days (empty cells before the 1st)
    ...
    let week = [];
    let cw = 0;
    const weekHeaders = ["CW", "Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"]; // Add headers to the grid
    calendarGrid.push(
      <React.Fragment key="header">
        {weekHeaders.map((day) => (
          <div
            key={day}
            className="font-bold text-gray-600 p-2 bg-blue-200 rounded"
          >
            {day}
          </div>
        ))}
    ...
    for (let i = 0; i < calendarCells.length; i++) {
      if (week.length === 0) {
        ...
        week.push(
          <div
            key={`cw-${cw}`}
            className="p-3 font-bold text-gray-600 bg-blue-200 rounded"
          >
            {cw}
          </div>,
        );
  ...
  return (
    <div>
      ...
      <div className="bg-blue-50 rounded-lg p-6 border-2 border-blue-300">
        ...
        {isLoading ? (
          <div className="text-center p-8 text-blue-600 col-span-full">
