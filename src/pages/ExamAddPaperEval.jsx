...
const ExamAddPaperEval = () => {
  ...
  const fetchSubjects = async (standard) => {
    try {
      ...
      if (
        ...
        // 1. data.subjects[0] is the document for standard "1"
        // 2. data.subjects[0].subjects is the Array of subject objects
        // 3. We map to get just the 'name' string from each object
        const subjectList = data.subjects[0].subjects.map((s) => s.name);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const result = await response.json(); // Check for server success field (fixed in controller)
      if (result.success) {
        alert("Paper assignment created successfully!");
