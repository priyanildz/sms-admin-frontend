            ...
            staffList.map((staff, index) => (
              <tr key={staff._id || index}>
                ...
                <td className="px-4 py-2 border">
                  {staff.fullName || staff.driverName || "N/A"}
                </td>
                <td className="px-4 py-2 border">
                  {staff.designation || (title === "Driver" ? "Driver" : "N/A")}
                </td>
                <td className="px-4 py-2 border">{staff.gender || "N/A"}</td>
                <td className="px-4 py-2 border">
                  {staff.contactNumber || staff.contactNo || "N/A"}
                </td>
                <td className="px-4 py-2 border">
                  <span
                    className={`font-semibold ${staff.status === "Active" ? "text-green-600" : "text-red-600"}`}
                  >
                    {staff.status || "N/A"}
                  </span>
                </td>
...
export default function TransportStaff() {
  ...
  const driversList = filteredList.filter(
    (staff) => staff.designation === "Driver",
  );
  const supervisorsList = filteredList.filter(
    (staff) => staff.designation === "Supervisor",
  );
