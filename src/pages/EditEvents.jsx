...
const getInitialSelectedParticipants = (originalEvent, allStudents) => {
  ...
  return originalEvent.participants
    .map((pId) => {
...
const EditEvents = () => {
  ...
  const [selectedManager, setSelectedManager] = useState(
    stringToArrayOfSelectOptions(originalEvent.managedby),
  );
  const [selectedStandard, setSelectedStandard] = useState(
    stringToArrayOfSelectOptions(originalEvent.standard),
  );
  const [selectedDivision, setSelectedDivision] = useState(
    stringToArrayOfSelectOptions(originalEvent.division),
  );
  ...
  const [formData, setFormData] = useState({
    ...
    date: originalEvent.date
      ? new Date(originalEvent.date).toISOString().split("T")[0]
      : "",
  });
  ...
  useEffect(() => {
    ...
    const currentSelectedIds = new Set(
      selectedParticipants.map((p) => p.value),
    );
    ...
    const missingSelectedStudents = selectedParticipants.filter(
      (p) => !newAvailableIds.has(p.value),
    );
    ...
    if (
      ...
      const initialSelections = getInitialSelectedParticipants(
        originalEvent,
        allStudents,
      );
      ...
      const updatedSelected = selectedParticipants
        .map((p) => mergedOptions.find((m) => m.value === p.value) || p)
        .filter((p) => p !== null);
  ...
  const validateForm = () => {
    ...
    const isFormValid =
      formData.eventname && formData.date && selectedManager.length > 0;
  ...
  const handleUpdate = async (e) => {
    ...
    if (!validateForm()) {
      setError(
        "Please fill all required fields (Name, Date, Manager) and ensure the date is current or future.",
      );
      return;
    }
  ...
  const handleDelete = async () => {
    if (
      !window.confirm(
        `Are you sure you want to permanently delete event "${originalEvent.eventname}"?`,
      )
    ) {
      return;
    }
    ...
    try {
      const storedUsername = localStorage.getItem("username") || "System_User";
      const storedRole = localStorage.getItem("role") || "admin";
      await axios.delete(`${API_BASE_URL}api/events/${originalEvent._id}`, {
        headers: {
          auth: "ZjVGZPUtYW1hX2FuZHJvaWRfMjAyMzY0MjU=",
          username: storedUsername,
          role: storedRole,
