...
export default function StaffAttendancePage() {
      ...
      await Promise.all(
        staffIds.map(async (staff) => {
          try {
            ...
            console.error(
              `Error fetching attendance for staff ${staff.staffid} in ${monthName}:`,
              err,
            );
          ...
          await Promise.all(
            months.map(async (month) => {
              try {
                ...
                console.error(
                  `Error fetching attendance for staff ${staff.staffid} in ${month} for yearly summary:`,
                  err,
                );
    ...
    if (isLoadingStaff || (isLoadingAttendance && view === "monthly")) {
      return (
        <div className="text-center py-10 text-blue-600">
          Loading staff list and attendance data...
        </div>
      );
    }
    if (error) {
  ...
  const renderYearlyView = () => {
    ...
    if (isLoadingStaff || (isLoadingAttendance && view === "yearly")) {
      return (
        <div className="text-center py-10 text-blue-600">
          Calculating yearly attendance totals...
        </div>
      );
    }
        ...
        <table className="table-auto border-collapse w-full min-w-max border border-gray-800 text-sm">
          <thead className="bg-blue-100 text-black">
            <tr className="border">
              ...
              {/* Change border to border-gray-800 */}
              <th className="px-4 py-2 text-center border border-gray-800 font-bold min-w-[80px]">
                Total
              </th>
              ...
              return (
                ...
                >
                  ...
                  {months.map((month, index) => {
                    ...
                    const monthlyCount =
                      summary[month] !== undefined ? summary[month] : 0;
                    return (
                      // Change border to border-gray-800
                      <td
                        key={index}
