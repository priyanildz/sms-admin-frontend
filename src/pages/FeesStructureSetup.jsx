...
const FeesStructureSetup = () => {
  ...
  const validateAssignFees = () => {
    ...
    const hasEmptyFields = assignFees.some(
      (fee) => !fee.category || !fee.amount,
    );
    if (hasEmptyFields) {
  ...
  const handleSaveAssign = async () => {
    ...
    try {
      ...
      const storedUsername = localStorage.getItem("username") || "System_User";
      const storedRole = localStorage.getItem("role") || "admin";
      const response = await axios.post(`${API_BASE_URL}api/add-fee`, payload, {
        headers: {
          auth: AUTH_HEADER,
          username: storedUsername,
          role: storedRole,
              ...
              <div className="space-y-6">
                ...
                {feeCategories.length === 0 && (
                  <p className="text-amber-600 bg-amber-50 border border-amber-200 rounded p-3 text-center">
                    No fee categories available. Please define fee categories
                    first.
                  </p>
                )}
