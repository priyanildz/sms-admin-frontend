...
export default function FeesManagement({ student }) {
  // Accept student prop here
  ...
  const fetchPayments = async (stdToFetch, divToFetch, studentName = null) => {
    ...
    try {
      ...
      const response = await axios.get(`${API_BASE_URL}api/payment-entries`, {
        headers: {
          auth: AUTH_HEADER,
      ...
      if (stdToFetch && divToFetch) {
        filteredEntries = filteredEntries.filter(
          (entry) =>
            String(entry.std) === String(stdToFetch) &&
            String(entry.div) === String(divToFetch),
        );
      }
