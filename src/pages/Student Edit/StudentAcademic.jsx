...
export default function StudentAcademic({ studentid, student }) {
  ...
  const fetchAcademicBlocks = async (stdToFetch, divToFetch) => {
    ...
    setAcademicBlocks([]);
    try {
      console.log("Fetching academic blocks for:", {
        std: stdToFetch,
        div: divToFetch,
      });
      ...
      const response = await axios.get(`${API_BASE_URL}api/academic-blocks`, {
        headers: {
          auth: AUTH_HEADER,
      ...
      const filteredData = response.data.filter((block) => {
        ...
        console.log(
          `Comparing DB item: (Std: ${block.standard}, Div: ${block.division}) | Match? ${isStdMatch && isDivMatch}`,
        );
  ...
  useEffect(() => {
    if (student && student.std && student.div) {
      console.log(
        "Student prop found in Academic, fetching automatically:",
        student,
      );
      ...
      <div className="max-w-4xl mx-auto">
        ...
        {!student ? (
          ...
          <div className="flex flex-col items-center gap-4 mb-8">
            {" "}
            {/* Use flex-col for centering */}
            {/* Student Name Display */}
            <div className="flex items-center gap-3 w-full max-w-sm">
              {" "}
              {/* Limit width */}
              <label className="text-gray-700 font-medium w-20 text-right">
                Name
              </label>
            ...
            {/* Std and Div Display */}
            <div className="flex gap-8 justify-center items-center w-full max-w-sm">
              {" "}
              {/* Limit width */}
              <div className="flex items-center gap-3 flex-1">
                <label className="text-gray-700 font-medium">Std</label>
                <div className="border border-gray-300 bg-gray-100 px-3 py-2 w-full rounded text-gray-700">
                  {" "}
                  {/* Use w-full */}
                  {student.std || "N/A"}
                </div>
            ...
            <tbody>
              {isLoading ? (
                <tr>
                  <td
                    colSpan="2"
                    className="border border-blue-500 px-4 py-4 bg-white text-center text-gray-500"
                  >
