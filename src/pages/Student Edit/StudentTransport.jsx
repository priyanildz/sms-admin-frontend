...
export default function TransportManagement({
  ...
  useEffect(() => {
    if (student && student.transport) {
      console.log(
        "Student prop found in Transport, populating form:",
        student.transport,
      );
      ...
      console.log(
        "Student prop found, but student.transport object is missing or empty.",
      );
  ...
  const handleSave = async (e) => {
    ...
    if (!student || !student._id) {
      setNotification({
        type: "error",
        message: "Cannot save. Student data not available.",
      });
      return;
    }
    if (!transportData.transportstatus) {
      setNotification({
        type: "error",
        message: "Transport Status is required.",
      });
      return;
    }

    // --- Frontend validation for required fields ---
    // 🟢 ADDED: Pickup Location check
    if (transportData.transportstatus === "yes" && !transportData.pickuppoint) {
      return setNotification({
        type: "error",
        message: 'Pickup Location is required when transport is "Yes".',
      });
    }
    // 🟢 ADDED: Drop Location check
    if (transportData.transportstatus === "yes" && !transportData.droppoint) {
      return setNotification({
        type: "error",
        message: 'Drop Location is required when transport is "Yes".',
      });
    }

    const isBus =
      transportData.transportstatus === "yes" &&
      transportData.modetransport === "bus";
    // Bus Number and Driver Name are intentionally kept not required here, following previous request.
    // --- End validation ---
    ...
    const updatePayload = {
      transport: {
        ...
        transportid:
          transportData.transportid || student.transport?.transportid,
      },
    };
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setNotification({
          type: "error",
          message: `Save failed: Server status ${response.status}`,
        });
      }
    } catch (error) {
    ...
    <div className="min-h-screen p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        ...
        {notification.message && (
          <div
            className={`p-3 mb-4 rounded-lg text-center ${notification.type === "success" ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800"}`}
          >
            {notification.message}
          </div>
        )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 divide-y-2 md:divide-y-0 md:divide-x-2 divide-blue-500">
              ...
              <div className="p-4 md:p-6">
                ...
                >
                  ...
                  {/* 🔴 REMOVED <option value="self">Self</option> */}
