...
//     const availableStaffs = useMemo(() => {
//         return staffs.filter(staff => {
//             // 1. Check if staff is already assigned to a classroom
//             const isAlreadyAssigned = assignments.some(a => a.staffid === staff._id);
//             const isCurrentStaff = editData && staff._id === editData.staffid;

//             if (isAlreadyAssigned && !isCurrentStaff) return false;

//             // 2. Filter by Grade Category based on selected standard
//             if (!formData.standard) return true; // Show all until std is selected

//             const std = formData.standard;
//             const preferredGrades = (staff.role?.preferredgrades || []).map(g => g.toLowerCase());

//             if (["Nursery", "Junior", "Senior"].includes(std)) {
//                 return preferredGrades.includes("pre-primary");
//             } else if (["1", "2", "3", "4", "5"].includes(std)) {
//                 return preferredGrades.includes("primary");
//             } else if (["6", "7", "8", "9", "10"].includes(std)) {
//                 return preferredGrades.includes("secondary");
//             }

//             return true;
//         });
//     }, [staffs, assignments, editData, formData.standard]);
...
// =========================================================================
const ClassAssignmentFormModal = ({ staffs, API_BASE_URL, AUTH_TOKEN, fetchAssignments, closeModal, editData }) => {
    const [formData, setFormData] = useState({
        standard: editData?.standard || "",
        division: editData?.division || "",
        staffid: editData?.staffid || "",
    ...
    useEffect(() => {
        const fetchEligible = async () => {
            ...
            try {
                const res = await fetch(`${API_BASE_URL}api/eligible-teachers/${formData.standard}`, {
                    headers: { "auth": AUTH_TOKEN }
                });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await fetch(`${API_BASE_URL}api/classrooms/${editData._id}`, {
                method: "PUT",
                headers: {
                    "Content-Type": "application/json",
                    "auth": AUTH_TOKEN,
    ...
    return (
        <div className="fixed inset-0 flex items-center justify-center z-[500]" style={{ backgroundColor: 'rgba(50, 50, 50, 0.5)' }}>
            <div className="bg-white rounded-xl shadow-2xl p-6 w-full max-w-lg mx-4">
                <div className="flex justify-between items-center mb-6 border-b pb-3">
                    <h2 className="text-xl font-bold text-gray-800">Change Class Teacher</h2>
                    <button onClick={closeModal} className="text-gray-500 hover:text-gray-800 text-2xl leading-none">&times;</button>
                </div>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Standard</label>
                            <input type="text" value={formData.standard} disabled className="mt-1 block w-full border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-500" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-gray-700">Division</label>
                            <input type="text" value={formData.division} disabled className="mt-1 block w-full border border-gray-300 rounded-md p-2 bg-gray-50 text-gray-500" />
                        </div>
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700">Class Teacher (Core Staff Only)</label>
                        <select 
                            name="staffid" 
                        ...
                        >
                            <option value="">{loadingTeachers ? "Loading core teachers..." : "Select Class Teacher"}</option>
...
export default function ClassAssign() {
    ...
    const handleDelete = async (id) => {
        if (!window.confirm("Are you sure you want to remove this classroom record?")) return;
        try {
            const response = await fetch(`${API_BASE_URL}api/classrooms/${id}`, { method: 'DELETE', headers: { auth: AUTH_TOKEN } });
            if (!response.ok) throw new Error("Delete failed.");
            fetchAssignments();
                ...
                <div className="overflow-x-auto border border-gray-200 rounded-xl shadow-sm"> 
                    <table className="min-w-full table-collapse text-sm"> 
                        ...
                        <tbody className="bg-white divide-y divide-gray-100">
                            {isLoading ? (
                                <tr><td colSpan="5" className="px-6 py-10 text-center text-gray-400">Loading assignments...</td></tr>
                            ) : filteredAssignments.map((assignment) => (
                                <tr key={assignment._id} className="hover:bg-blue-50 transition-colors">
                                    <td className="px-6 py-4 font-medium text-gray-900">{assignment.standard}</td>
                                    <td className="px-6 py-4 text-gray-600">{assignment.division}</td>
                                    <td className="px-6 py-4">
                                        <span className="bg-green-100 text-green-800 text-xs font-semibold px-2.5 py-1 rounded-full border border-green-200">
                                            {getStaffName(assignment.staffid)}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 text-gray-600">{assignment.studentcount}</td>
                                    <td className="px-6 py-4">
                            ...
                            {!isLoading && filteredAssignments.length === 0 && (
                                <tr>
                                    <td colSpan="5" className="px-6 py-10 text-center text-gray-500 italic">No classroom assignments found. Ensure core subjects are allotted first.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
