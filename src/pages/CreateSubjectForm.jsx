  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (existing) {
        setLoading(false);
        return alert(
          `Subjects for Standard ${selectedStd} already exist. Please use Edit instead.`,
        );
      }
      ...
      alert(
        err.response?.data?.message ||
          "Error saving subjects. Ensure backend models are updated.",
      );
    } finally {
      setLoading(false);
    }
                    ...
                    <td className="px-6 py-3 border border-gray-300">
                      <div className="flex flex-col items-start justify-center gap-1 mx-auto w-fit">
                        {["Compulsory", "Optional"].map((t) => (
                          <label
                            key={t}
                            className={`flex items-center gap-2 cursor-pointer text-sm ${!row.isSelected ? "opacity-50 pointer-events-none" : ""}`}
                          >
                            <input
                              type="radio"
                              name={`type-${idx}`}
