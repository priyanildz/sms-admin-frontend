...
const AcademicEditSubject = () => {
  ...
  const initialSubjectsMasterList = [
    ...
    {
      name: "Social Science",
      subOptions: ["History", "Civics", "Political Science", "Geography"],
    },
  ...
  const handleUpdate = async () => {
    ...
    // Validation
    const incomplete = selectedSubjects.find(
      (s) => !s.type || s.nature.length === 0,
    );
    if (incomplete)
      return alert(`Please select Type and Nature for ${incomplete.name}`);
    ...
    try {
      ...
      const storedRole = localStorage.getItem("role") || "admin";
      await axios.put(
        `${API_BASE_URL}api/subjects/${recordId}`,
        {
      ...
      <div className="bg-white rounded-2xl shadow p-6">
        <div className="p-6 space-y-6">
          ...
          <div className="flex items-center gap-2">
            ...
            <input
              className="border border-gray-300 px-4 py-2 rounded-md shadow-sm w-48 bg-gray-50 outline-none"
              value={selectedStd}
              readOnly
            />
          </div>
          <div className="overflow-x-auto shadow-md rounded-lg border border-gray-200">
            <table className="min-w-full border-collapse">
              <thead className="bg-blue-100 text-black">
                <tr>
                  <th className="text-center px-6 py-3 border border-gray-300 font-bold uppercase tracking-wider text-xs">
                    Subjects Name
                  </th>
                  <th className="text-center px-6 py-3 border border-gray-300 font-bold uppercase tracking-wider text-xs">
                    Subjects Type
              ...
              <tbody className="bg-white">
                {loading ? (
                  ...
                  formData.map((row, idx) => (
                    <tr
                      key={idx}
                      className={`border-b hover:bg-gray-50 transition-colors ${!row.isSelected ? "bg-gray-50 opacity-80" : ""}`}
                    >
                      ...
                      <td className="px-6 py-3 border border-gray-300">
                        <div
                          className={`flex flex-col items-start justify-center gap-1 mx-auto w-fit ${!row.isSelected ? "pointer-events-none" : ""}`}
                        >
                      ...
                      <td className="px-6 py-3 border border-gray-300">
                        <div
                          className={`flex flex-col items-start justify-center gap-1 mx-auto w-fit ${!row.isSelected ? "pointer-events-none" : ""}`}
                        >
                          {["Language", "Theory", "Practical", "Activity"].map(
                            (n) => (
                              <label
                                key={n}
                                className="flex items-center gap-2 cursor-pointer text-xs"
                              >
                                <input
                                  ...
                                  onChange={() =>
                                    handleCheckboxChange(idx, "nature", n)
                                  }
                      ...
                      <td className="px-6 py-3 border border-gray-300">
                        <div
                          className={`grid grid-cols-2 gap-x-4 gap-y-1 text-left w-fit mx-auto ${!row.isSelected ? "pointer-events-none" : ""}`}
                        >
                          {(row.availableSubOptions || []).map((sub) => (
                            ...
                            >
                              <input
                                ...
                                onChange={() =>
                                  handleCheckboxChange(idx, "subSubjects", sub)
                                }
                                className="w-3 h-3 text-blue-600 rounded"
                              />{" "}
                              {sub}
                          ...
                          {row.availableSubOptions.length === 0 && (
          ...
          <div className="flex justify-end gap-4 mt-4">
            <button
              onClick={() => navigate("/academics-subject-creation")}
              className="px-6 py-2 border border-gray-300 rounded-md hover:bg-gray-50 font-medium text-sm"
            >
              Back
            </button>
            <button
              ...
              disabled={loading}
              className="px-6 py-2 bg-blue-500 text-white rounded-md shadow hover:bg-blue-600 transition-all font-medium text-sm"
            >
