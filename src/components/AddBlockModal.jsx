...
const AddBlockModal = ({
  ...
  const handleChange = (field) => (e) => {
    setFormData({ ...formData, [field]: e.target.value }); // Clear error for this field when user starts typing
    if (errors[field]) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      console.log("Submitting form data:", formData); // Prepare data for API - match your backend expected format
      const apiData = {
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        resetForm(); // Notify parent component to refresh data
        if (onBlockAdded) {
      ...
      console.error("Error submitting form:", error); // Handle different types of errors
      if (error.response) {
