...
const VehicleStaffDetailsModal = ({ show, onClose, staff, API_BASE_URL }) => {
  if (!show || !staff) return null; // Initialize form data based on the staff prop

  const [editFormData, setEditFormData] = useState(staff);
  const [isEditing, setIsEditing] = useState(false);
  const [isSaving, setIsSaving] = useState(false);
  const [errors, setErrors] = useState({}); // File-related states

  const [staffPhoto, setStaffPhoto] = useState(null);
  const [imagePreview, setImagePreview] = useState(
    staff.photoUrl || staff.photo || null,
  ...
  const validateEditForm = () => {
    ...
    if (
      ...
      (!editFormData.licenseNumber || editFormData.licenseNumber.trim() === "")
    ) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const storedRole = localStorage.getItem("role") || "admin"; // NOTE: In a real app, image upload logic (staffPhoto) would go here.
      ...
      const response = await axios.put(
        `${API_BASE_URL}api/edit-staff/${staff._id}`,
        finalData,
        {
          headers: {
  ...
  const headerText = isEditing
    ? `Edit ${staff.designation} Details`
    : `View ${staff.designation} Details`; // Reusable Input/Select components tailored for staff fields
  ...
  const CustomInput = ({
    ...
    if (options) {
      // Renders Select field if options are provided
      return (
            ...
            <div className="w-full flex flex-col p-6 rounded-lg shadow-lg bg-white self-start">
              <div className="flex flex-col items-center">
                ...
                <div className="w-full border-t pt-4 mt-4 flex justify-center items-center gap-3">
                  <span
                    className={`text-sm font-bold ${!getIsActive(editFormData.status) ? "text-red-600" : "text-gray-400"}`}
                  >
                    Resigned
                  </span>
                  <label
                  ...
                  >
                    ...
                    />
                    <div
                      className={`w-11 h-6 bg-gray-200 rounded-full peer peer-checked:bg-green-500 after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full`}
                    ></div>
                  </label>
                  <span
                    className={`text-sm font-bold ${getIsActive(editFormData.status) ? "text-green-600" : "text-gray-400"}`}
                  >
                    Active
                  </span>
                </div>
