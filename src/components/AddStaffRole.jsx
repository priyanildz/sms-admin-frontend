...
const AddStaffRoleModal = ({ isOpen, onClose, staffId = null }) => {
  ...
  const [fetchingStaff, setFetchingStaff] = useState(false); // Moved options to be accessible in useEffect
  ...
  const fetchStaffList = async () => {
    try {
      ...
      setFetchingStaff(true); // FIX 1: Using imported API_BASE_URL
      ...
      setStaffList(mappedStaff); // If a staffId isn't provided, default to the first staff member in the list
      ...
      <div className="relative bg-white border border-gray-300 shadow-xl rounded-lg p-6 w-96">
        ...
        <div className="space-y-4">
          ...
          <SelectField
            ...
            options={roleOptions}
            onChange={(value) =>
              handleSpecificInputChange("roleassigned", value)
            }
          />
        </div>
