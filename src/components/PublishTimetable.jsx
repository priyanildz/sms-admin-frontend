...
const PublishTimetable = ({ handleCancelPublish }) => {
  ...
  const AUTH_HEADER = `ZjVGZPUtYW1hX2FuZHJvaWRfMjAyMzY0MjU=`; // fetch subjects based on standard
  ...
  useEffect(() => {
    ...
    fetchSubjects();
  }, [formData.standard]); // fetch allotments based on standard + division
  ...
  useEffect(() => {
    ...
    fetchAllotments();
  }, [formData.standard, formData.division]); // input handling

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData((prev) => ({ ...prev, [name]: value })); // Clear success/error messages when user starts typing
