...
//     icon={link.icon}
//     text={link.text}
//     isActive={currentRoute === link.to || activeSection === link.id} // Add ID check
//     onClick={(e) => {
//       if (link.id) { // If it has an ID, it's a state-based link
//         e.preventDefault();
...
//                 icon={link.icon}
//                 text={link.text}
//                 isActive={currentRoute === link.to || activeSection === link.id}
//                 onClick={(e) => {
//                   if (link.id) {
//                     e.preventDefault();
//                     onSectionChange(link.id);
...
// function SidebarLink({ to, icon, text, isActive, showChevron = true, onClick }) {
//   return (
//     <Link
//       to={to || "#"}
//       onClick={onClick}
...
export default function MainLayout({ children, activeSection, onSectionChange }) {
  ...
  // Helper to determine which "Back" link to show
  const renderBackLink = () => {
    if (currentRoute === "/dashboard") return null;

    // Logic for Student Section back navigation
    if (currentRoute.includes("/students/edit-student/")) {
      return (
        <SidebarLink
          to="/students"
          icon={faArrowLeft}
    ...
    // Logic for Staff Section back navigation
    if (currentRoute.includes("/edit-staff/") || currentRoute.startsWith("/staff-")) {
      return (
        <SidebarLink
          to="/staff"
          icon={faArrowLeft}
    ...
    // Default back link for general pages
    return backToDashboardLink.map(({ to, icon, text }, index) => (
      <SidebarLink
        key={index}
        to={to}
        icon={icon}
